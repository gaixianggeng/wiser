...
int init_database(wiser_env *env, const char *db_path) {
    ...
    sqlite3_exec(env->db, "CREATE UNIQUE INDEX token_index ON tokens(token);", NULL, NULL, NULL);

    sqlite3_exec(env->db, "CREATE UNIQUE INDEX title_index ON documents(title);", NULL, NULL, NULL);

    sqlite3_prepare(env->db, "SELECT id FROM documents WHERE title = ?;", -1, &env->get_document_id_st, NULL);
    sqlite3_prepare(env->db, "SELECT title FROM documents WHERE id = ?;", -1, &env->get_document_title_st, NULL);
    sqlite3_prepare(env->db, "INSERT INTO documents (title, body) VALUES (?, ?);", -1, &env->insert_document_st, NULL);
    sqlite3_prepare(env->db, "UPDATE documents set body = ? WHERE id = ?;", -1, &env->update_document_st, NULL);
    sqlite3_prepare(env->db, "SELECT id, docs_count FROM tokens WHERE token = ?;", -1, &env->get_token_id_st, NULL);
    sqlite3_prepare(env->db, "SELECT token FROM tokens WHERE id = ?;", -1, &env->get_token_st, NULL);
    sqlite3_prepare(env->db,
                    "INSERT OR IGNORE INTO tokens (token, docs_count, postings)"
                    " VALUES (?, 0, ?);",
                    -1, &env->store_token_st, NULL);
    sqlite3_prepare(env->db, "SELECT docs_count, postings FROM tokens WHERE id = ?;", -1, &env->get_postings_st, NULL);
    sqlite3_prepare(env->db, "UPDATE tokens SET docs_count = ?, postings = ? WHERE id = ?;", -1,
                    &env->update_postings_st, NULL);
    sqlite3_prepare(env->db, "SELECT value FROM settings WHERE key = ?;", -1, &env->get_settings_st, NULL);
    sqlite3_prepare(env->db, "INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?);", -1,
                    &env->replace_settings_st, NULL);
    sqlite3_prepare(env->db, "SELECT COUNT(*) FROM documents;", -1, &env->get_document_count_st, NULL);
    sqlite3_prepare(env->db, "BEGIN;", -1, &env->begin_st, NULL);
    sqlite3_prepare(env->db, "COMMIT;", -1, &env->commit_st, NULL);
    sqlite3_prepare(env->db, "ROLLBACK;", -1, &env->rollback_st, NULL);
    return 0;
}
 ...
 */
int db_get_document_title(const wiser_env *env, int document_id, const char **title, int *title_size) {
    int rc;
    ...
    if (rc == SQLITE_ROW) {
        if (title) {
            *title = (const char *)sqlite3_column_text(env->get_document_title_st, 0);
        }
        if (title_size) {
            *title_size = (int)sqlite3_column_bytes(env->get_document_title_st, 0);
        }
    }
    return 0;
}
 ...
 */
int db_add_document(const wiser_env *env, const char *title, unsigned int title_size, const char *body,
 ...
 */
int db_get_token_id(const wiser_env *env, const char *str, unsigned int str_size, int insert, int *docs_count) {
    int rc;
    if (insert) {
        sqlite3_reset(env->store_token_st);
        sqlite3_bind_text(env->store_token_st, 1, str, str_size, SQLITE_STATIC);
    ...
    sqlite3_bind_text(env->get_token_id_st, 1, str, str_size, SQLITE_STATIC);
 ...
 */
int db_get_token(const wiser_env *env, const int token_id, const char **const token, int *token_size) {
    int rc;
 ...
 */
int db_get_postings(const wiser_env *env, int token_id, int *docs_count, void **postings, int *postings_size) {
    int rc;
 ...
 */
int db_get_settings(const wiser_env *env, const char *key, int key_size, const char **value, int *value_size) {
    int rc;

    sqlite3_reset(env->get_settings_st);
    sqlite3_bind_text(env->get_settings_st, 1, key, key_size, SQLITE_STATIC);
 ...
 */
int db_replace_settings(const wiser_env *env, const char *key, int key_size, const char *value, int value_size) {
    int rc;
    sqlite3_reset(env->replace_settings_st);
    sqlite3_bind_text(env->replace_settings_st, 1, key, key_size, SQLITE_STATIC);
    sqlite3_bind_text(env->replace_settings_st, 2, value, value_size, SQLITE_STATIC);
query:
 ...
 * @param[in] env 存储着应用程序运行环境的结构体
 */
int begin(const wiser_env *env) { return sqlite3_step(env->begin_st); }

/**
 * 提交事务
 * @param[in] env 存储着应用程序运行环境的结构体
 */
int commit(const wiser_env *env) { return sqlite3_step(env->commit_st); }

/**
 * 回滚事务
 * @param[in] env 存储着应用程序运行环境的结构体
 */
int rollback(const wiser_env *env) { return sqlite3_step(env->rollback_st); }
