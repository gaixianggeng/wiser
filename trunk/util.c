...
void timeval_to_str(struct timeval *clock, char *const buffer) {
    ...
    sprintf(buffer, "%04d/%02d/%02d %02d:%02d:%02d.%06d", result.tm_year + 1900, result.tm_mon + 1, result.tm_mday,
            result.tm_hour, result.tm_min, result.tm_sec, (int)(clock->tv_usec / 100000));
}
 ...
 */
double timeval_to_double(struct timeval *tv) { return ((double)(tv->tv_sec) + (double)(tv->tv_usec) * 0.000001); }
