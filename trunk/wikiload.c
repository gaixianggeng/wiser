...
static void XMLCALL end(void *user_data, const XML_Char *el) {
    ...
    switch (p->status) {
        ...
        case IN_PAGE_REVISION_TEXT:
            if (!strcmp(el, "text")) {
                p->status = IN_PAGE_REVISION;
                if (p->max_article_count < 0 || p->article_count < p->max_article_count) {
 ...
 * @retval 4 解析XML文件失败
 */
int load_wikipedia_dump(wiser_env *env, const char *path, add_document_callback func, int max_article_count) {
    FILE *fp;
    int rc = 0;
    XML_Parser xp;
    ...
    if (!(fp = fopen(path, "rb"))) {
        print_error("cannot open wikipedia dump xml file(%s).", strerror(errno));
        rc = 2;
        goto exit;
    }
    ...
    while (1) {
        ...
        if (done || (max_article_count >= 0 && max_article_count <= wp.article_count)) {
            break;
        }
    }
exit:
