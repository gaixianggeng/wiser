...
static int search_phrase(const query_token_hash* query_tokens, doc_search_cursor* doc_cursors) {
    ...
    if ((cursors = (phrase_search_cursor*)malloc(sizeof(phrase_search_cursor) * n_positions))) {
        ...
        while (cursors[0].current) {
            ...
            if (next_rel_position > rel_position) {
                ...
                while (cursors[0].current && (*cursors[0].current - cursors[0].base) < next_rel_position) {
 ...
 * @return 得分
 */
static double calc_tf_idf(const query_token_hash* query_tokens, doc_search_cursor* doc_cursors,
                          const int n_query_tokens, const int indexed_count) {
    int i;
    const query_token_value* qt;
 ...
 */
void free_token_positions_list(token_positions_list* list) { free_postings_list((postings_list*)list); }
 ...
 */
void search_docs(wiser_env* env, search_results** results, query_token_hash* tokens) {
    int n_tokens;
...
int split_query_to_tokens(wiser_env* env, const UTF32Char* text, const unsigned int text_len, const int n,
                          query_token_hash** query_tokens) {
    return text_to_postings_lists(env, 0, /* 将document_id设为0 */
                                  text, text_len, n, (inverted_index_hash**)query_tokens);
}
