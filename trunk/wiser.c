 ...
 * @retval 0 成功
 */
static int init_env(wiser_env* env, int ii_buffer_update_threshold, int enable_phrase_search, const char* db_path) {
    int rc;
 ...
 * @param[in] env 存储着应用程序运行环境的结构体
 */
static void fin_env(wiser_env* env) { fin_database(env); }
 ...
 */
static void parse_compress_method(wiser_env* env, const char* method, int method_size) {
    ...
    if (!method || !method_size || MEMSTRCMP(method, method_size, "golomb")) {
        ...
        print_error("invalid compress method(%.*s). use golomb instead.", method_size, method);
        env->compress = compress_golomb;
    }
    switch (env->compress) {
        case compress_none:
        ...
        case compress_golomb:
            db_replace_settings(env, "compress_method", sizeof("compress_method") - 1, "golomb", sizeof("golomb") - 1);
            break;
    }
}
